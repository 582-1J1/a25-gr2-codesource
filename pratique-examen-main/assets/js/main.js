...
const oTextes = {
    fr: {
        ...
        msgJetonsEpuises: "Tu n'as plus de jetons",
        btnRejouer: "Rejouer",
        btnRecommencer: "Retour à l'intro"
    },
    en: {
        titre: "B L A C K J A C K",
        ...
        msgJetonsEpuises: "You have no tokens left",
        msgCroupierCrevé: "Dealer busted. You win...",
        btnRejouer: "Play Again",
        btnRecommencer: "Back to Intro"
    },
    es: {
        titre: "B L A C K J A C K",
...
// Son pour  gain
const oSonGain = new Audio("assets/sons/gain.wav");
oSonGain.volume = 1;
// Son pour perte
const oSonPerte = new Audio("assets/sons/perte.wav");
oSonPerte.volume = 0.8;
...
oCanvas.addEventListener("click", function (evt) {
    ...
    if (sEtat === "intro") {
        ...
        // Gérer le clic sur les boutons d'achats de jetons
        if(x>100 && x<250 && y>100 && y<150) {
            // nombre de jetons = 50
            nJetonsJoueur = 50;
            console.log("Clic sur le bouton 50 jetons");
            
        }
        if(x>300 && x<450 && y>100 && y<150) {
            // nombre de jetons = 100
            nJetonsJoueur = 100;
        }
        if(x>500 && x<650 && y>100 && y<150) {
            // nombre de jetons = 200
            nJetonsJoueur = 200;
        }
 ...
 * @param {Boolean} bRetourIntro : indique si on veut retourner à l'intro ou à ^
 *                                la partie. 
 * @return {void}
 */
function reinitialiserMains(bRetourIntro=false) {
    sEtat = "partie";
    ...
    // Si le paramètre bRetourIntro est true, il faut mettre sEtat à "partie" et
    // aussi réinitialiser la valeur de la variable nJetonsJoueur
    if(bRetourIntro == true) {
        nJetonsJoueur = 100;
        sEtat = "intro";
    }
}
...
function introJeu() {
    ...
    if (nPosXTexte < oDimCanvas.centre) {
        ...
        // Boutons pour achat de jetons
        dessinerBouton(100, 100, 150, 50, "#49F", "50 jetons");
        dessinerBouton(300, 100, 150, 50, "#49F", "100 jetons");
        dessinerBouton(500, 100, 150, 50, "#49F", "200 jetons");
...
function afficherResultat() {
    ...
    // Afficher état des jetons si <=0 et sinon afficher résultat de la partie
    if(nJetonsJoueur > 0) {
        ...
        sMessage = oTextes[langue].msgJetonsEpuises;
        oCtx.fillText(sMessage, oDimCanvas.centre, 225);
        dessinerBouton(350, 250, 200, 50, "red", oTextes[langue].btnRecommencer); 
    }
}
